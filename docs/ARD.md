...
## ADR-032 — Repo-wide scoped-correctness `ruff` lint (pre-commit + CI); per-ticket gates unchanged; mypy/coverage/style still excluded
**Status:** accepted — operator decision 2026-06-13. Amends the "no ruff/mypy gate" wording (root `CLAUDE.md` § Ticket creation; `.claude/CLAUDE.md` Policy 11/12) for a SCOPED correctness lint only.
**Context:** A 2026-06-13 GitHub code-scanning pass surfaced ~20 code-quality findings in shipped product code (unused imports, bare `except: pass`, `open()` without a context manager, an f-string-without-placeholders) that were only caught AFTER push, by CodeQL. The project had deliberately excluded ruff/mypy/coverage from its gates to avoid style churn + type-annotation overhead — but "no linter at all" pushed the cheap, deterministic correctness classes to a slow post-push tool, the opposite of shift-left. The operator chose to keep direct-push-to-main (no branch protection), which makes a LOCAL pre-commit hook the real pre-push catch.
**Decision:** Adopt **ruff** as an always-on, repo-wide hygiene layer restricted to **CORRECTNESS rules only** — `select = ["F", "E722", "SIM115", "B"]` (pyflakes unused/redefinition/f-string bugs · bare-except · open-without-context-manager · flake8-bugbear likely-bugs). **Explicitly NOT** style/formatting (no `ruff format`, no `E`/`W` rules beyond `E722`), **NOT** type-checking (no mypy), **NOT** coverage. Wired two ways: (1) a **pre-commit hook** (`.pre-commit-config.yaml`, ruff-pre-commit pinned `v0.15.17`; `pre-commit install` once per clone) — the pre-push catch for the direct-push flow; (2) a **CI `lint` job** (`.github/workflows/ci.yml`, `ruff==0.15.17` pinned) — the post-push signal. Rules live in root `ruff.toml`; the deliberately-vulnerable fixtures (`evals/corpus`, `docs/research`) are excluded (same rationale as the CodeQL `paths-ignore` + the dependabot scoping). Version pinned per ADR-006.
**Per-ticket gates UNCHANGED:** the ticket-template acceptance gates remain **pytest on the touched package + `validate_manifest.py` + `claude plugin validate`**. ruff is a repo-wide lint, NOT a per-ticket verification gate — a ticket's Quality-Gates section does not list it.
**Rationale:** ruff catches the common correctness classes in <1s locally, BEFORE they reach CodeQL post-push; CodeQL still owns the deeper analysis (unused module globals, redundant comparisons, the security queries) that ruff's correctness subset does not cover — the two are complementary. Restricting to a correctness subset preserves the original convention's intent (no style/type churn) while closing the shift-left gap; under direct-push the pre-commit hook (not branch protection) is the enforcement surface.
**Supersedes:** amends the "real gates … NOT ruff/mypy/coverage" wording in root `CLAUDE.md` § Ticket creation + `.claude/CLAUDE.md` Policy 11 → "NOT mypy/coverage/style; a scoped correctness ruff runs repo-wide (ADR-032), not as a per-ticket gate." ADR-013's plan-first / verification-per-task discipline + the per-ticket gate list are otherwise unchanged.
**Alternatives rejected:** (a) full ruff (style + import-sorting + formatting) — the churn the original convention rightly avoided. (b) mypy/type-checking — high annotation cost on stdlib-first scripts. (c) CI-only (no pre-commit) — under direct-push that only catches post-push; the operator chose pre-commit + CI for a true pre-push catch. (d) CodeQL-on-PR alone — slow, and at the default threshold it gates only higher-severity findings, annotating (not blocking) the note-level classes.
**References:** operator decision 2026-06-13 (this session); the 2026-06-13 code-scanning cleanup (commits `824b43e`/`ec58db6`); `ruff.toml`, `.pre-commit-config.yaml`, `.github/workflows/ci.yml` (`lint` job); ADR-006 (pin AND verify), ADR-013 (plan-first / per-task verification), Policy 11/12 (root `CLAUDE.md`); `.github/codeql/codeql-config.yml` (parallel fixture exclusion).
