...
the work to the **white-hacker** agent (`agents/white-hacker.md`) running the **sec-patch**
skill (`skills/sec-patch/SKILL.md`) — that skill is the behavioral source of truth.
(Plugin-relative paths; resolved at runtime under `${CLAUDE_PLUGIN_ROOT}`.)
...
`skills/sec-patch/patch-state-schema.json`): **build → original PoC stops → existing tests
