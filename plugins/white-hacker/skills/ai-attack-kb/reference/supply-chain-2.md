...
detections:
  ...
  - "layered crypto-exfil: an install/CI script pairing a symmetric cipher (Node createCipheriv, e.g. aes-256-cbc) with an asymmetric key-wrap (publicEncrypt, RSA-OAEP) before exfil — the double-wrap shape (deps-scan S6 HIGH at ≥2 dangerous-API hits; dual-use — legit hybrid encryption matches the same shape, so S6 flags it for human triage, it is not a block) [primary-sourced: https://nodejs.org/api/crypto.html#cryptocreatecipherivalgorithm-key-iv-options , https://nodejs.org/api/crypto.html#cryptopublicencryptkey-buffer]"
  - "npm-publish-impersonation: a non-npm-CLI script setting npm-CLI request headers (npm-command/npm-session/npm-scope) while contacting registry.npmjs.org to auto-publish — the header+registry-host co-occurrence fingerprint [primary-sourced: https://github.com/npm/npm-registry-fetch , https://docs.npmjs.com/cli/v11/using-npm/registry]"
